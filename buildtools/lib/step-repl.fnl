;; Title: Step REPL
;; Description: A fennel REPL that can be stepped through
;; Author: AlexJGriffith
;; URL: https://codeberg.org/alexjgriffith/step-repl
;; Version: 1.0 2024-11-20
;; Licence: GPL3+
;; Dependencies: fennel

;; This module is a fork of the repl module in fennel v 1.5.1

;; It is ment to unwrap the repl loop function that relies on yeilding
;; into a closure that gets called when new input is available.

;; by default the repl will function the same however if
;; you pass in opts.stepped-repl as true executing the repl will not
;; loop
;; you will also skip the readChunk callback and instead pass in the
;; string you would like to evaluate to the repl as a function.
;; (local myrepl (step-repl opts))
;; (myrepl "(+ 1 1)")
;; (myrepl "(+ 1")
;; (myrepl ")")
...
;; Example Usage
;; (local repl (require :step-repl))

;; (local opts {:readChunk nil ;; skipped
;;              :onValues (fn [vals]
;;                          (io.write (table.concat vals "\t"))
;;                          (io.write "\n"))
;;              :onError (fn [_errtype err]
;;                         (io.write (table.concat [err] "\t"))
;;                         (io.write "\n"))
;;              :moduleName "fennel"
;;              :stepped-repl true})

;; (local stdio (repl opts))

;; (var ss 0)
;; (while true
;;   (io.write  (if (> ss 0) ".." ">> "))
;;   (io.flush)
;;   (set ss (stdio  (.. (tostring (io.read)) "\n"))))

;; requires fennel to be available
...
(fn run-command-step [input read quit quit-arg env on-values on-error scope chars opts]
  (let [command-name (input:match ",([^%s/]+)")]
    (case (. commands command-name)
      command (command env read on-values on-error scope chars opts)
      _ (when (and (not= command-name :exit) (not= command-name :return))
          (on-values ["Unknown command" command-name])))
    (when (and (not= :exit command-name) (= :return command-name))
      (quit quit-arg))
    command-name))
...
(fn repl [?options]
  ...
  (let [old-root-options utils.root.options
        ...
        (byte-stream clear-stream) (parser.granulate #(callbacks.readChunk $ input))
        chars []
      ...
      (let [(ok parser-not-eof? form) (pcall read)
        ...
        (if (not ok)
            ...
            (when not-eof?
              ...
              (if exit-next?
                  (quit env.___replLocals___.*1)
                  (loop))))))

    (fn step [input_string]
      (set input (.. (or input "") input_string "\n"))
      (fn reset-input []
        (set input nil)            
        0)
      (fn signal-repl-end []
        (set input nil)
        -1)
      (fn incomplete-input [] 1)
      (each [k (pairs chars)]
        (tset chars k nil))
      ;;(reset)
      (let [read (parser.parser input)
            (ok parser-not-eof? form) (pcall read)
            badend? (and parser-not-eof?
                         (= :string (type parser-not-eof?))
                         (parser-not-eof?:match "expected closing delimiter"))
            src-string input
            ;; Work around a bug introduced in lua-readline 3.2
            readline-not-eof? (or (not readline) (not= src-string "(null)"))
            not-eof? (and readline-not-eof? parser-not-eof?)]
        ;; (-> {: ok : parser-not-eof? : form : readline-not-eof? : not-eof?} (view) (print))
        (if (not ok)
            (if badend?
                (incomplete-input)
                (do
            ...
            (do
              (let [command-name
                    (run-command-step src-string read quit env.___replLocals___.*1 env
                                      callbacks.onValues callbacks.onError
                                      opts.scope chars opts)]
                (match command-name
                  :exit (signal-repl-end)
                  _ (reset-input))))
            (do
              (when not-eof?
                (case-try (pcall compiler.compile form
